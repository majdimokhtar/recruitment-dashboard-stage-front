...
const ResetPassword = () => {
  ...
  useEffect(() => {
    ...
    if (!router.query.uidb64 || !router.query.token) {
      // should probably be redirected to 404 Not Found
      router.push('/login');
    }
  ...
  const onSubmit = data => {
    ...
    axiosInstance
      .post(`https://127.0.0.1:8000/api/auth/reset-password/${uidb64}/${token}`, { new_password: newPassword })
      .then(() => {
        router.push('/login')
      })
      .catch((err) => {
